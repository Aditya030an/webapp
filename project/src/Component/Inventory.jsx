...
const Inventory = () => {
  ...
  const fetchInventoryData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/report/inventory`);
  ...
  useEffect(() => {
    if (!selectedMonth && !selectedYear) {
      ...
      const filtered = inventoryData.filter((order) => {
        ...
        const matchMonth = selectedMonth ? d.getMonth() + 1 === Number(selectedMonth) : true;
        const matchYear = selectedYear ? d.getFullYear() === Number(selectedYear) : true;
  ...
  const filteredTotal = filteredInventory.reduce((sum, entry) => sum + entry.total, 0);
    ...
    <div>
      <div className="min-h-screen bg-gray-100 px-4 md:px-6 py-6">
        ...
        <div className="bg-white shadow-sm px-6 py-3 rounded-lg mb-6 flex space-x-4 overflow-x-auto">
          {categories.map((cat) => (
            <Link key={cat.name} to={cat.path} className="px-4 py-2 rounded-full font-medium bg-gray-200 text-gray-700 hover:bg-blue-100">
              {cat.name}
            </Link>
          ))}
        </div>
            ...
            <tbody>
              {items.map((item, idx) => (
                <tr key={idx} className="border-b">
                  <td className="p-2">
                    <input type="text" value={item.name} onChange={(e) => handleItemChange(idx, "name", e.target.value)} className="w-full border p-1 rounded" />
                  </td>
                  <td className="p-2">
                    <input type="number" value={item.quantity} onChange={(e) => handleItemChange(idx, "quantity", e.target.value)} className="w-full border p-1 rounded" />
                  </td>
                  <td className="p-2">
                    <input type="number" value={item.unitPrice} onChange={(e) => handleItemChange(idx, "unitPrice", e.target.value)} className="w-full border p-1 rounded" />
                  </td>
                  <td className="p-2">₹{(item.quantity * item.unitPrice).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>

          <button onClick={addItem} className="mb-4 bg-blue-100 text-blue-700 px-3 py-1 rounded">
            + Add Item
          </button>
...
<div className="flex items-center justify-between p-2">
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredInventory.map((order) => (
          <div key={order._id} className="bg-white border border-gray-200 rounded-xl shadow p-4">
            <h2 className="text-lg font-semibold text-indigo-600">Purchase Details</h2>
            <p className="text-sm text-gray-600 mb-2">Date: {new Date(order.createdAt).toLocaleDateString()}</p>
            <h3 className="font-semibold text-gray-800 mb-1">Items:</h3>
            <ul className="list-disc pl-5 text-sm text-gray-700 mb-2">
              {order.items.map((item) => (
                <li key={item._id}>{item.name} - {item.quantity} pcs @ ₹{item.unitPrice}</li>
              ))}
            </ul>
            <div className="text-right font-bold text-green-700">Total: ₹{order.total}</div>
          </div>
        ))}
      </div>
    </div>
