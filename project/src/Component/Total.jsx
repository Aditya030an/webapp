...
const Total = () => {
  ...
  useEffect(() => {
    const fetchCategoryTotal = async () => {
      try {
        ...
        const expensesFiltered = expensesData.data.filter((e) => isMatch(e.date));
        const inventoryFiltered = inventoryData.data.filter((i) => isMatch(i.createdAt));
        ...
        const salaryFiltered = salaryData.data.filter((s) =>
          s.employees.some((e) => {
            ...
            const monthMatch = selectedMonth ? Number(month) === Number(selectedMonth) : true;
            const yearMatch = selectedYear ? Number(year) === Number(selectedYear) : true;
            return monthMatch && yearMatch;
          })
        );
        ...
        const totals = {
          ...
          inventoryTotal: inventoryFiltered.reduce((sum, i) => sum + i.total, 0),
          totalInventory: inventoryFiltered.length,
          rentTotal: rentFiltered.reduce((sum, r) => sum + r.amount, 0),
          totalRent: rentFiltered.length,
          salaryTotal: salaryFiltered.reduce((sum, s) => sum + s.totalSalary, 0),
  ...
  const generatePDF = () => {
    const monthName = selectedMonth ? new Date(0, selectedMonth - 1).toLocaleString("default", { month: "long" }) : "All Months";
    const year = selectedYear || "All Years";
    const total = categoryTotal?.billTotal + categoryTotal?.expensesTotal + categoryTotal?.inventoryTotal + categoryTotal?.rentTotal + categoryTotal?.salaryTotal;
        ...
        <div className="max-w-5xl mx-auto bg-white p-8 rounded-xl shadow-md">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-xl font-semibold text-blue-600">
              ₹{categoryTotal?.billTotal + categoryTotal?.expensesTotal + categoryTotal?.inventoryTotal + categoryTotal?.rentTotal + categoryTotal?.salaryTotal}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-blue-50 p-6 rounded-lg shadow-sm">
              ...
              <p className="text-xl font-bold text-blue-900">₹{categoryTotal?.billTotal}</p>
              <p className="text-sm text-gray-500">Total Bills: {categoryTotal?.totalBill}</p>
            </div>

            <div className="bg-red-50 p-6 rounded-lg shadow-sm">
              <h3 className="text-lg font-semibold text-red-700">Expenses</h3>
              <p className="text-xl font-bold text-red-900">₹{categoryTotal?.expensesTotal}</p>
              <p className="text-sm text-gray-500">Total Expenses: {categoryTotal?.totalExpenses}</p>
            </div>

            <div className="bg-green-50 p-6 rounded-lg shadow-sm">
              <h3 className="text-lg font-semibold text-green-700">Inventory</h3>
              <p className="text-xl font-bold text-green-900">₹{categoryTotal?.inventoryTotal}</p>
              <p className="text-sm text-gray-500">Total Inventory: {categoryTotal?.totalInventory}</p>
            </div>

            <div className="bg-yellow-50 p-6 rounded-lg shadow-sm">
              <h3 className="text-lg font-semibold text-yellow-700">Rent</h3>
              <p className="text-xl font-bold text-yellow-900">₹{categoryTotal?.rentTotal}</p>
              <p className="text-sm text-gray-500">Total Rent: {categoryTotal?.totalRent}</p>
            </div>

            <div className="bg-purple-50 p-6 rounded-lg shadow-sm">
              <h3 className="text-lg font-semibold text-purple-700">Salary</h3>
              <p className="text-xl font-bold text-purple-900">₹{categoryTotal?.salaryTotal}</p>
              <p className="text-sm text-gray-500">Total Salary: {categoryTotal?.totalSalary}</p>
            </div>
          </div>
        </div>
      </div>
